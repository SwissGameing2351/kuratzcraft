...
hunger_ng = {
    ...
    settings = {
        hunger_bar = {
            image = get('hunger_bar_image', 'sick.png'),
            use = minetest.is_yes(get('use_hunger_bar', true)),
            force_builtin_image = get('force_builtin_image', true),
        },
        timers = {
            heal = tonumber(get('timer_heal', 11)),
            starve = tonumber(get('timer_starve', 7)),
            basal_metabolism = tonumber(get('timer_basal_metabolism', 0.4)),
            movement = tonumber(get('timer_movement', 0.1))
        },
        hunger = {
            timeout = tonumber(get('hunger_timeout', 4)),
            persistent = minetest.is_yes(get('hunger_persistent', true)),
            start_with = tonumber(get('hunger_start_with', 15)),
            maximum = tonumber(get('hunger_maximum', 20))
        }
    },
    effects = {
        heal = {
            above = tonumber(get('heal_above', 15)),
            amount = tonumber(get('heal_amount', 1)),
        },
        starve = {
            below = tonumber(get('starve_below', 1)),
            amount = tonumber(get('starve_amount', 1)),
            die = minetest.is_yes(get('starve_die', true))
        },
        disabled_attribute = 'hunger_ng:hunger_disabled'
    },
    costs = {
        base = tonumber(get('cost_base', 0.001)),
        dig = tonumber(get('cost_dig', 0.002)),
        place = tonumber(get('cost_place', 0.002)),
        movement = tonumber(get('cost_movement', 0.001))
    }
}
