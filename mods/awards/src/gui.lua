...
function awards.get_formspec(name, to, sid)
	...
	if sdef and sdef.secret and not sitem.unlocked then
		...
		formspec = formspec .. "textarea[0.5,1;4.8,1;;" ..
			string.format(status, minetest.formspec_escape(title)) ..
			";]"

		if sdef and sdef.icon then
			formspec = formspec .. "image[0.45,0;1,1;" .. sdef.icon .. "]"  -- adjusted values from 0.6,0;3,3
		end
		...
		if sitem.progress then
			...
			formspec = formspec .. "background[0,7.7;" .. barwidth ..",0.4;awards_progress_gray.png;false]"
			formspec = formspec .. "background[0,7.7;" .. (barwidth * perc) ..",0.4;awards_progress_green.png;false]"
			if label then
				formspec = formspec .. "label[1.6,7.6;" .. minetest.formspec_escape(label) .. "]"
			end
		end

		if sdef and sdef.description then
			formspec = formspec .. "box[-0.05,1.5;3.9,6;#000]"
			formspec = formspec	.. "textarea[0.25,1.5;3.9,4.2;;" ..
					minetest.formspec_escape(sdef.description) .. ";]"
		end
	end

	-- Create list box
	formspec = formspec .. "textlist[4,0;3.8,7.5;awards;"
	local first = true
